...
function goSpotify() {
	spotify.request("https://api.spotify.com/v1/search?q=track:"+titleName+"&type=track&market=US")
		.then(function(data) {
			if (data.tracks.total === 0) {											// If there aren't any tracks...
				console.log("Sorry, we can't find that song");							// Then sorry.
			}
				else {																// Otherwise...
					...
					if (data.tracks.items[0].preview_url === null) {				// If there is no preview track...
						console.log("Sorry, we don't have any sample.");				// Then sorry.
					}
						else {														// Otherwise...
